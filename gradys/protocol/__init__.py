...
Protocols are created by subclassing the [IProtocol][gradys.protocol.interface.IProtocol] interface. The IProtocol interface
...
[IProvider][gradys.protocol.interface.IProvider] instance that is injected into the protocol when it is instantiated. The
...
Messages are sent using the [CommunicationCommand][gradys.protocol.messages.communication.CommunicationCommand] class. It
...
The protocol can affect its node's mobility by sending [MobilityCommand][gradys.protocol.messages.mobility.MobilityCommand] via
